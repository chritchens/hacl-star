...
let rec write_vale_mem 
  ...
  : Tot heap (decreases %[sub length i]) =
...
let correct_down_p_frame mem (addrs:addr_map) (heap:heap) (p:b8) : Lemma
  (forall p'. disjoint p.b p'.b /\ correct_down_p mem addrs heap p' ==>       
      (let length = B.length p.b in
      let contents = B.as_seq mem p.b in
      let addr = addrs p in
      let new_heap = write_vale_mem contents length addr 0 heap in
      correct_down_p mem addrs new_heap p')) = 
  let rec aux (p':b8) : Lemma 
    (disjoint p.b p'.b /\ correct_down_p mem addrs heap p' ==> (let length = B.length p.b in
      let contents = B.as_seq mem p.b in
      let addr = addrs p in
      let new_heap = write_vale_mem contents length addr 0 heap in
      correct_down_p mem addrs new_heap p')) =
...
private let add_buffer_domain (b:b8) (addrs:addr_map) (accu:Set.set int) : GTot (s:Set.set int{
  forall x. Set.mem x s <==> (Set.mem x accu \/ (x >= addrs b /\ x < addrs b + B.length b.b))}) =
  let rec aux (j:nat{j <= B.length b.b}) (acc:Set.set int{forall x. Set.mem x acc <==>
    Set.mem x accu \/ (x >= addrs b /\ x < addrs b + j)}) :
    GTot (s:Set.set int{forall x. Set.mem x s <==>
    Set.mem x accu \/ (x >= addrs b /\ x < addrs b + B.length b.b)}) 
    (decreases %[B.length b.b - j]) = 
    if j >= B.length b.b then acc else begin
...
private let rec addrs_set_aux (ptrs:list b8)
			      ...
			      (accu:Set.set int{forall (x:int). not (Set.mem x accu) <==>
			        (forall (b:b8{List.memP b ps}). x < addrs b \/ x >= addrs b + B.length b.b)}) :
			 GTot (s:Set.set int{forall (x:int). not (Set.mem x s) <==>
			        (forall (b:b8{List.memP b ptrs}). x < addrs b \/ x >= addrs b + B.length b.b)}) =
  match ps2 with
  | [] -> accu
  | a::q -> 
	...
	x < addrs b \/ x >= addrs b + B.length b.b)) = ()
    in
    Classical.forall_intro aux;
...
let write_buffer_vale (a:b8) (heap:heap) (mem:HS.mem) (addrs:addr_map) =
  let length = B.length a.b in
  let contents = B.as_seq mem a.b in
  let addr = addrs a in
...
let rec down_mem_aux (ptrs:list b8{list_disjoint_or_eq ptrs})
  ...
  match ps with
    ...
    | a::q ->
      ...
      assert (forall (p:b8). List.memP p accu ==> disjoint_or_eq p.b a.b);
...
let rec frame_down_mem_aux (ptrs:list b8{list_disjoint_or_eq ptrs})
  ...
  | a::q ->
    ...
    correct_down_p_cancel mem addrs h a;
    correct_down_p_frame mem addrs h a;
    assert (forall p. List.memP p accu ==> disjoint_or_eq p.b a.b);
    domain_write_buffer a h mem addrs;
...
let get_seq_heap_as_seq (heap1 heap2:heap) (addrs:addr_map) (mem:HS.mem) (b:b8) : Lemma
  (requires correct_down_p mem addrs heap1 b /\
    (forall x. x >= addrs b /\ x < addrs b + B.length b.b ==> heap1.[x] == heap2.[x]))
  (ensures B.as_seq mem b.b == get_seq_heap heap2 addrs b) =
  assert (Seq.equal (B.as_seq mem b.b) (get_seq_heap heap2 addrs b))
...
let rec up_mem_aux (heap:heap)
  ...
  : GTot (m':HS.mem{correct_down m' addrs ptrs heap /\ list_live m' ptrs}) =
    ...
    | a::q ->
      ...
      B.g_upd_seq_as_seq a.b s m;
      let m' = B.g_upd_seq a.b s m in
...
let rec down_up_identity_aux
  ...
  | a::q ->
      let s = get_seq_heap heap addrs a in
      let m' = B.g_upd_seq a.b s m in
      B.lemma_g_upd_with_same_seq a.b m;
      assert (Seq.equal s (B.as_seq m a.b));
...
let correct_down_p_same_sel (b:b8) (mem:HS.mem) (addrs:addr_map) (heap1 heap2:heap) (x:int) : Lemma
  (requires (x >= addrs b /\ x < addrs b + B.length b.b 
  ...
  (ensures Map.sel heap1 x == Map.sel heap2 x) = 
    ...
    assert (heap1.[x] == UInt8.v (Seq.index (B.as_seq mem b.b) i))
...
let g_upd_tot_correct_down_invariant
  ...
  (ensures (
    let m' = B.g_upd_seq b.b (get_seq_heap heap2 addrs b) mem in
    forall p. (p =!= b /\ List.memP p ptrs) ==> correct_down_p m' addrs heap1 p)
  ) = 
    B.g_upd_seq_as_seq b.b (get_seq_heap heap2 addrs b) mem
...
let g_upd_tot_correct_down
  ...
  (heap:heap)
  (b:b8{B.live mem b.b}) :
  Lemma (correct_down_p (B.g_upd_seq b.b (get_seq_heap heap addrs b) mem) addrs heap b) =
  B.g_upd_seq_as_seq b.b (get_seq_heap heap addrs b) mem
...
let rec update_buffer_up_mem_aux
  ...
  (requires (forall x. x < addrs b \/ x >= addrs b + B.length b.b ==> heap1.[x] == heap2.[x]) /\
    (List.memP b accu ==> B.as_seq mem b.b == get_seq_heap heap2 addrs b) /\  
  ...
  (~(List.memP b accu) ==> up_mem_aux heap2 addrs ptrs ps accu mem ==
    B.g_upd_seq b.b (get_seq_heap heap2 addrs b) mem))
  (decreases ps)
  ...
  = match ps with
   ...
   | a::q ->
     ...
     B.g_upd_seq_as_seq a.b s mem;         
     let m' = B.g_upd_seq a.b s mem in
     let m_f = up_mem_aux heap2 addrs ptrs q (a::accu) m' in
     // Why is this not verifying, even with an assume?
     assume (up_mem_aux heap2 addrs ptrs (a::q) accu mem == m_f);
     admit();
     if StrongExcludedMiddle.strong_excluded_middle (a == b) then (
       if StrongExcludedMiddle.strong_excluded_middle (List.memP b accu) then (
         admit();
         B.lemma_g_upd_with_same_seq a.b mem;       
